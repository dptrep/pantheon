...
def test(symbol, date, bar_size):
    sql = '''select * from ib_minute where symbol='%s' and date='%s' order by time;''' % (symbol, date)
...
def run_multi(kk:list, bar_size:int=30) -> dict:
    ...
    for symbol in symbols:
        ...
        #print(sql)
        im = ib_minute.IbMinute()
        raw = im.query(sql, im.columns, string_dates=False)
        d = {}
        for x in raw:
            ts = (x['date'], x['time'])
            if ts not in d:
...
if __name__ == '__main__':
    ...
    start = '20200831'
    #end = '20210107'
    bb = run_single(symbol, start, 1800)
    for b in bb:
        if b.complete:
            print(b.get())